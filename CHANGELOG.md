...
- **24-hour cache TTL for static sensors** - Applied TTL caching to firmware version sensors (Inverter, Communication Module, Battery), serial number sensor, and device type code sensor. These values rarely or never change, so caching them for 24 hours significantly reduces unnecessary Modbus traffic.
...
- **Derived sensor mutates controller private attributes** (`modbus_controller.py`, `sensors/sungrow_derived_sensor.py`, `0a6f87b`) - Derived sensor directly accessed and mutated private attributes (`_sw_version`, `_model`) on the controller object, violating encapsulation. Root cause: tight coupling between derived sensor and controller internals. Fixed by adding `set_sw_version()` and `set_model()` setter methods to ModbusController and updating derived sensor to use these methods instead of direct mutation.

- **Service handler write result discarded** (`__init__.py`, `6374ca1`) - Service calls to `sungrow_write_holding_register` always appeared to succeed from Home Assistant's perspective, even when the underlying Modbus write failed. Root cause: the return value from `write_with_logging()` was discarded. Fixed by checking the return value and raising `HomeAssistantError` on failure, allowing users and automations to properly detect write failures.

- **Connect return value ignored in read/write methods** (`modbus_controller.py`, `0ed3545`) - When the Modbus device was offline, read/write operations proceeded anyway because the return value of `connect()` was ignored. This caused repeated exceptions and log spam during connection failures. Root cause: `_execute_write_holding_register()`, `_execute_write_holding_registers()`, `async_read_input_register()`, and `async_read_holding_register()` all called `await self.connect()` but never checked the boolean return value. Fixed by adding `if not await self.connect(): return None` checks that skip the operation and log a debug message when not connected.

- **Switch entity optimistic state update causes UI/device desync** (`sensors/sungrow_binary_sensor.py`, `6cdf33c`) - Switch turn_on/turn_off used fire-and-forget pattern (via `hass.create_task`) that immediately updated local state, even if the Modbus write would fail. This caused the UI to show the switch as toggled while the inverter never received the command. Root cause: sync `turn_on()`/`turn_off()` methods couldn't await the async write operation. Fixed by converting to async `async_turn_on()`/`async_turn_off()` methods that properly await the Modbus write and only update state after confirmed success. Raises `HomeAssistantError` on write failure.

- **Number entity optimistic state update causes UI/device desync** (`sensors/sungrow_number_sensor.py`, `0e49714`) - Number entity `set_native_value` used fire-and-forget pattern that immediately updated local state without waiting for write confirmation. This caused the UI to show the new value while the inverter never received it. Root cause: sync `set_native_value()` method couldn't await the async write operation. Fixed by converting to async `async_set_native_value()` that properly awaits the Modbus write and only updates state after confirmed success. Raises `HomeAssistantError` on write failure.
