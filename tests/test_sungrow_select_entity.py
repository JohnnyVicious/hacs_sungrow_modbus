...
async def test_set_register_bit_enforces_conflicts_and_requires(mock_hass, mock_controller):
    ...
    # 6 and 11 are on
    with patch(
        "custom_components.sungrow_modbus.sensors.sungrow_select_entity.cache_get",
...
async def test_set_register_bit_with_requires(mock_hass, mock_controller):
    ...
    with patch("custom_components.sungrow_modbus.sensors.sungrow_select_entity.cache_get", return_value=0):
        entity = SungrowSelectEntity(
            mock_hass, mock_controller, {"register": register, "name": "Work Mode", "entities": []}
        )
