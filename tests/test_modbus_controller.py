...
class TestModbusControllerTCP(IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.manager_patcher = patch("custom_components.sungrow_modbus.modbus_controller.ModbusClientManager")
...
class TestModbusControllerSerial(IsolatedAsyncioTestCase):
    ...
    async def test_serial_async_read_input_register_success(self):
        ...
        self.assertEqual([42], result)
        # Serial: device_id is passed explicitly (pymodbus 3.x requirement)
        self.mock_client.read_input_registers.assert_called_once_with(address=100, count=1, device_id=1)
...
class TestModbusControllerInitialization(unittest.TestCase):
    ...
    def test_tcp_without_host_raises_error(self):
        ...
        with self.assertRaises(ValueError) as context:
            ModbusController(
                hass=self.hass, connection_type=CONN_TYPE_TCP, port=502, inverter_config=self.inverter_config
            )
    ...
    def test_serial_without_port_raises_error(self):
        ...
        with self.assertRaises(ValueError) as context:
            ModbusController(
                hass=self.hass, connection_type=CONN_TYPE_SERIAL, baudrate=9600, inverter_config=self.inverter_config
            )
