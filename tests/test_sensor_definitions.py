...
from custom_components.sungrow_modbus.sensor_data.hybrid_sensors import hybrid_sensors, hybrid_sensors_derived
...
class TestSensorDefinitions(unittest.TestCase):
    def check_group(self, name, sensor_groups, derived_sensors):
        for group in sensor_groups:
            ...
            self.assertTrue(
                entities[0].get("register"), f"{name}: Entity {entities[0].get('name')} has no registers defined"
            )
