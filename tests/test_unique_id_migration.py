...
async def test_unique_id_migration(hass: HomeAssistant, auto_enable_custom_integrations):
    ...
    # Old ID format: sungrow_modbus_{host}_{slave}_{sensor_unique}
    # (Assuming SungrowSensorGroup/Derived used generic logic or we focus on one example)
    ...
    # For 'SH10RT', let's pick a sensor. 'active_power' usually has unique 'active_power'.

    registry = er.async_get(hass)
    old_unique_id = f"{DOMAIN}_{host}_{slave}_active_power"
    reg_entry = registry.async_get_or_create(
        "sensor", DOMAIN, old_unique_id, suggested_object_id="sungrow_active_power", config_entry=entry
    )
    ...
    mock_controller.inverter_config.type = "hybrid"  # Sungrow SH series are hybrids
    ...
    mock_controller.poll_speed = {"fast": 5, "normal": 15, "slow": 30}

    # The plan is to implement migration in async_setup_entry.

    with (
        patch("custom_components.sungrow_modbus.get_controller", return_value=mock_controller),
        patch("custom_components.sungrow_modbus.ModbusController", return_value=mock_controller),
    ...
    # 4. Verify Migration
    # New ID format: sungrow_modbus_{host}_{port}_{slave}_{sensor_unique}
