...
async def test_setup_entry(hass: HomeAssistant):
    ...
    with patch("custom_components.sungrow_modbus.modbus_controller.ModbusController.connect", return_value=True), \
         ...
         patch("custom_components.sungrow_modbus.modbus_controller.ModbusController.async_read_holding_register", return_value=[1,2,3]):
...
async def test_setup_entry_connection_failure(hass: HomeAssistant):
    ...
    with patch("custom_components.sungrow_modbus.modbus_controller.ModbusController.connect", side_effect=ConnectionError):
